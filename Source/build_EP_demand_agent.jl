...
function add_EP_demand_agent_to_planner!(planner::Model, id::String, mod::Model,
    ...
    # D_EP_bar = load_profile × peak: physical upper bound on EP consumption.
    D_EP_bar = ts[:LOAD_EP] .* p[:D_EP_bar]
    A_EP = p[:A_EP]    # intercept of inverse demand for EP
    B_EP = p[:B_EP]    # slope of inverse demand for EP
    ...
    # Welfare contribution = quadratic utility U(d) = A_EP·d - (B_EP/2)·d².
